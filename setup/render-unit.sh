...
mdns_wants=""
# The "*.local" alias published here is used by OmniRoute and remote-setup's
# own mDNS URLs too (see scripts/print-endpoints.sh), not just llm-server's --
# so this must keep publishing even when llm_server.enabled is false. Only
# the readiness probe and the advertised port need to switch targets, since
# llm-server's own /health never answers in that mode.
if command -v avahi-publish >/dev/null 2>&1; then
    ...
    if [ "$llm_server_enabled" = "true" ]; then
        health_probe="${curl_path} -fsS -o /dev/null http://127.0.0.1:${port}/health"
        advertise_port="$port"
    else
        omniroute_port="$(yq -r '.omniroute.port' "$CONFIG_PATH")"
        # No nested bash -c here: this expression is already interpolated
        # inside the outer ExecStartPre's single-quoted `bash -c '...'`, so
        # a second pair of single quotes would terminate that outer string
        # early and corrupt the generated unit.
        health_probe="(exec 3<>/dev/tcp/127.0.0.1/${omniroute_port}) 2>/dev/null"
        advertise_port="$omniroute_port"
    fi
    cat > "$mdns_unit" <<EOF
# Generated by render-unit.sh. Publishes only after the stack is healthy.
[Unit]
Description=llm-env mDNS publication
...
[Service]
Type=simple
ExecStartPre=/usr/bin/bash -c 'i=0; while [ \$\$i -lt ${LLM_ENV_HEALTH_TIMEOUT_SECONDS} ]; do ${health_probe} && exit 0; i=\$\$((i + 1)); sleep 1; done; exit 1'
ExecStart=/usr/bin/bash ${publish_script} ${mdns_name}.local ${mdns_name} ${advertise_port}
Restart=on-failure
RestartSec=2
EOF
    chmod 600 "$mdns_unit"
