...
.PHONY: help setup-dev shell clean cache-status validate
...
help:
	...
	@echo "╠════════════════════════════════════════════════════════════╣"
	@echo "║ make setup-dev      Build complete LLM environment         ║"
...
setup-dev:
	@echo "🚀 Starting LLM environment setup with docker-like caching..."
	@bash setup-dev.sh
...
shell:
	@if distrobox list | grep -q "$(CONTAINER_NAME)"; then \
		...
		echo "❌ Container $(CONTAINER_NAME) not found. Run 'make setup-dev' first."; \
		exit 1; \
	fi
...
clean-cache:
	@echo "🗑️  Clearing all build checkpoints..."
	@rm -rf "$(CHECKPOINTS)"
	@echo "✓ Checkpoints cleared. Next run will rebuild from scratch."

clean:
	@echo "⚠️  This will remove the entire LLM environment!"
